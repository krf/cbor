 ...
 */
static double decode_float_half(unsigned char *halfp)
{
 ...
 */
static void dump_memory(const unsigned char *data, size_t size)
{
    if (!data || !size) {
        return;
    }
        ...
        case CBOR_ARRAY: {
            ...
            while (is_indefinite ? !cbor_at_break(stream, offset) : i < array_length) {
                ...
                if (inner_read_bytes == 0) {
                    printf("Failed to read array item at position %zu", i);
                    break;
                }
        ...
        case CBOR_MAP: {
            ...
            while (is_indefinite ? !cbor_at_break(stream, offset) : i < map_length) {
                ...
                if (key_read_bytes == 0 || value_read_bytes == 0) {
                    printf("Failed to read key-value pair at position %zu", i);
                    break;
                }
...
void cbor_stream_decode(cbor_stream_t *stream)
{
    ...
    while (offset < stream->pos) {
        ...
        if (read_bytes == 0) {
            printf("Failed to read from stream at offset %zu, start byte 0x%02X\n", offset, stream->data[offset]);
